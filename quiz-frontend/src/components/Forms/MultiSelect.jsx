...
const MultiSelect = ({ id }) => {
  ...
  useEffect(() => {
    const loadOptions = () => {
      ...
      if (select) {
        const newOptions = Array.from(select.options).map(option => ({
          value: option.value,
  ...
  const selectedValues = () => selected.map(option => options[option].value).join(', ');
          ...
          <div className="relative z-20 inline-block w-full">
            <div className="relative flex flex-col items-center">
              ...
              <div className="w-full px-4">
                <div className={`max-h-select absolute top-full left-0 z-40 w-full overflow-y-auto rounded bg-white shadow dark:bg-form-input ${isOpen() ? '' : 'hidden'}`} ref={dropdownRef}>
